...
export function importSystemWorkflowSubscriptions(vraHost?: vCACCAFE.VCACHost, configurationFile?: MimeAttachment, overwriteExisting?: boolean): void {var tenant = vraHost.tenant;
System.log("Importing system workflow subscriptions using connection to: " + tenant + " with user: " + vraHost.username);

var content = configurationFile.content;

var subscriptions = JSON.parse(content);

var service = vraHost.createEventClient().getEventSystemWorkflowSubscriptionService();

for (var sub of subscriptions) {
	var existing = service.getSystemSubscription(sub.id);
	...
	if (!existing) {
	    ...
	    existing = service.getSystemSubscriptions(request).getContent()[0];
	}
	if (existing) {
		...
		if (overwriteExisting) {
			...
			service.deleteSystemSubscription(existing.getId());
		} else {
			continue;
		}
	}
	...
	workflowSubscription.setFailOnError(sub.failOnError);
	...
	System.log("Creating system workflow subscription " + workflowSubscription.getName());
